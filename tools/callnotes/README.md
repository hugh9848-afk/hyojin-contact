...
전화기가 두 대입니다. **이 자동화는 학원폰(S22) 하나에만** 겁니다.

| | 학원폰 (S22) | 개인폰 (S25) |
|---|---|---|
| 통화 자동 녹음 | **켬** (①) | 끔 — 건드리지 않습니다 |
| Autosync 앱 | **설치·설정** (②) | 설치하지 않습니다 |
| 통화기록 화면 | `?v=열쇠` 로 한 번 열고 홈 화면에 추가 (⑥) | 열지 않는 쪽을 권합니다\* |

\* 열면 학부모 통화 내용이 개인폰에도 남습니다. 밖에서 확인할 일이 잦으면 여셔도 되지만,
그만큼 개인정보가 한 곳 더 늘어난다는 뜻입니다.

③④⑦ 은 **맥미니**에서, ⑤⑥ 은 브라우저만 있으면 **어디서든** 하면 됩니다.
...
역시 **학원폰(S22)에서** 합니다. 플레이스토어에서 **Autosync for Google Drive** (만든 곳: MetaCtrl) 를 설치합니다.
